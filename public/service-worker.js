var version = '2016-3-28-18:58:55';

self.addEventListener('install', function(event) {
  console.log('[ServiceWorker] Installed version', version);
  event.waitUntil(
    fetch('./mymodule.js').then(function(response) {
      return caches.open(version).then(function(cache) {
        console.log('[ServiceWorker] Cached mymodule.js for', version);
        // Important to `return` the promise here to have `skipWaiting()`
        // fire after the cache has been updated.
        return cache.put('mymodule.js', response);
      });
    }).then(function() {
  ...
  if (event.request.url.includes('/mymodule.js')) {
    ...
    event.respondWith(
      caches.open(version).then(function(cache) {
        cache.keys().then(keys => console.log('[ServiceWorker] keys:', keys));
        return cache.match('mymodule.js').then(function(response) {
          if (!response) {
